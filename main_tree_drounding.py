"""
Modified main.py for running D-Rounding experiments on tree topology with clean switches.
...
@hydra.main(version_base=None, config_path="settings", config_name="main_tree_drounding")
def run(config: DictConfig):
    """
    Run the VNE simulation with tree topology and clean switches.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running D-Rounding solver on tree topology with clean switches (routing-only)")
