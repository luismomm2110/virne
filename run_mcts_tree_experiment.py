#!/usr/bin/env python3
"""
Standalone script to run MCTS experiments on tree topology with 5 different seeds.

This script runs the Monte Carlo Tree Search (MCTS) solver on a tree topology with
...
simulations to find optimal virtual network embeddings. It balances exploration
and exploitation using the UCB1 algorithm.

Reference:
    Soroush Haeri et al. "Virtual Network Embedding via Monte Carlo Tree Search".
...
def run_experiment(seed):
    """
    Run a single MCTS experiment with the given seed.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Running MCTS Tree Experiment - Seed {seed}")
    print(f"{'='*60}\n")
    ...
    cmd = [
        ...
        f"experiment.run_id=mcts_tree_seed_{seed}"
    ]
...
def main():
    ...
    """
    print("\n" + "="*60)
    print("MCTS Tree Topology Experiment Suite")
    print("="*60)
    print("\nConfiguration:")
    print("  - Solver: Monte Carlo Tree Search (MCTS)")
    ...
    print("  - Computation Budget: 5 iterations per node")
    print("  - Exploration Constant: 0.5")
    print("="*60 + "\n")

    # Check if we're in the correct directory
    if not Path("main_tree_mcts.py").exists():
        print("Error: main_tree_mcts.py not found!")
    ...
    if len(failed_seeds) == 0:
        ...
        print("\nResults saved in:")
        print("  virne/mcts/mcts_tree_seed_*/records/")
        print("\nNext steps:")
        print("  1. Analyze results across all seeds")
        print("  2. Compare acceptance rates and R2C ratios")
        print("  3. Compare with MIP, GA, D-Rounding, and PL-Rank solver results")
    else:
