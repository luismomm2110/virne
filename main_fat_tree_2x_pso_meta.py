"""
PSO-Meta experiments on Fat-Tree 2x topology (32 nodes).

This script runs the PSO-Meta solver on the larger Fat-Tree 2x topology
for comparative analysis with PL-Rank.

Usage:
    python main_fat_tree_2x_pso_meta.py p_net_setting=fat_tree_2x_p_net_setting \
                                        v_sim_setting=v_sim_200_requests \
                                        solver.solver_name=pso_meta \
                                        experiment.seed=0
"""
...
@hydra.main(version_base=None, config_path="settings", config_name="main_fat_tree_2x_pso_meta")
def run(config: DictConfig):
    """
    Run the VNE simulation with Fat-Tree 2x topology.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running PSO-Meta solver on Fat-Tree 2x topology (32 nodes)")
