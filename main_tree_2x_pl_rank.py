"""
PL-Rank experiments on Tree 2x topology (64 nodes).

This script extends the standard main.py to run the PL-Rank solver
on the larger Tree 2x topology for comparative analysis.

Usage:
    python main_tree_2x_pl_rank.py p_net_setting=tree_2x_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_tree_2x_pl_rank")
def run(config: DictConfig):
    """
    Run the VNE simulation with Tree 2x topology.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running PL-Rank solver on Tree 2x topology (64 nodes)")
