"""
SA-Meta experiments on Fat-Tree 2x topology (32 nodes).

This script runs the SA-Meta solver on the larger Fat-Tree 2x topology
for comparative analysis with PL-Rank.

Usage:
    python main_fat_tree_2x_sa_meta.py p_net_setting=fat_tree_2x_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_fat_tree_2x_sa_meta")
def run(config: DictConfig):
    """
    Run the VNE simulation with Fat-Tree 2x topology.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running SA-Meta solver on Fat-Tree 2x topology (32 nodes)")
