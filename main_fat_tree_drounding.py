"""
Modified main.py for running D-Rounding experiments on fat-tree topology with clean switches.
...
This script supports both D-Rounding (d_round) and R-Rounding (r_round) algorithms.

Usage:
    # For D-Rounding:
    python main_fat_tree_d_round.py p_net_setting=fat_tree_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_fat_tree_drounding")
def run(config: DictConfig):
    """
    Run the VNE simulation with fat-tree topology and clean switches.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print(f"Running {config.solver.solver_name.upper()} solver on fat-tree topology with clean switches (routing-only)")
