#!/usr/bin/env python3
"""
Step 3: Train Decision Trees for Multi-Objective VNE Algorithm Selection (Version 2)

Adapted to work with existing dataset structure:
- Uses 'best_overall' as the primary label (closest to oracle)
- Also trains trees for cost optimization objective
...
def print_summary(results):
    """Print summary of all trained trees."""
    ...
    objectives = {
        'overall': 'Overall Best Algorithm (Oracle)',
    }

    print("\nModel Performance by Objective:")
    print(f"{'Objective':<35} {'Accuracy':<12} {'F1-Score':<12} {'Train Acc':<12}")
    print("-" * 70)

    for obj_key in ['overall']:
        if obj_key in results:
            metrics = results[obj_key]
            print(f"{objectives[obj_key]:<35} "
...
if __name__ == '__main__':
    ...
    all_algos = pd.concat([train_df['best_overall'].dropna()]).unique()
    ...
    # Using what's available in the dataset
    objectives = {
        'overall': ('Overall Best Algorithm (Oracle)', 'best_overall'),
    }

    # Train decision trees (one per objective)
    models = {}
    results = {}
    ...
    print("  - Select objective at runtime (overall or cost)")
