...
\begin{itemize}
    \item Um conjunto diverso de simulações utilizando a plataforma \textit{ViRNE}, executando sete diferentes algoritmos de VNE (exatos, heurísticos e meta-heurísticos) em topologias \textit{Tree} e \textit{Fat-Tree} com parâmetros variados e diferentes características de carga, resultando em 8000 VNRs únicas para treino.
...
O restante deste artigo está organizado da seguinte forma: a Seção~\ref{sec:RefTeo} apresenta o referencial teórico necessário para compreensão do problema de VNE e técnicas de aprendizado de máquina usando árvores de decisão, como o \textit{XGBoost}; a Seção~\ref{sec:RelatedWork} discute trabalhos relacionados em VNE, aprendizado de máquina aplicado a redes e seleção de algoritmos, destacando as contribuições diferenciais deste trabalho; a Seção~\ref{sec:Proposal} descreve a metodologia proposta, incluindo o processo de geração de dados de simulação, extração de \textit{features}, treino do modelo \textit{XGBoost} e estratégia de avaliação; a Seção~\ref{sec:Results} apresenta os resultados experimentais, analisando o desempenho do seletor dinâmico em comparação com algoritmos \textit{baseline}; finalmente, a Seção~\ref{sec:Conclusion} conclui o artigo e delineia direções futuras de pesquisa.
...
Para realizar a experimentação e coleta de dados necessários ao treino do seletor de algoritmos, foi utilizado o \textit{ViRNE}\cite{virne}, uma plataforma unificada de \textit{benchmark} para \textit{Virtual Network Embedding}.
...
Além disso, o \textit{ViRNE} inclui funcionalidades de \textit{logging} detalhado e extração de métricas, facilitando a coleta das \textit{features} e resultados necessários para o treino de modelos de aprendizado de máquina.
...
\subsection{Algoritmos de Virtual Network Embedding}

Fischer et al.~\cite{fischer2013vne-survey} apresentam um \textit{survey} abrangente sobre VNE, categorizando os algoritmos em exatos, heurísticos e meta-heurísticos. Os autores destacam que métodos exatos baseados em Programação Linear Inteira garantem otimalidade mas sofrem com escalabilidade, sendo impraticáveis para redes de grande porte. Esta limitação motiva o desenvolvimento de abordagens alternativas que balanceiem qualidade de solução e eficiência computacional. Contudo, a maioria dos algoritmos propostos na literatura mantém estratégias fixas para todas as requisições, não considerando a heterogeneidade de características das VNRs ou o estado dinâmico da rede física.

\subsection{Aprendizado de Máquina Aplicado a VNE}

Cao et al.~\cite{cao2018nfv-deeprl} aplicam aprendizado por reforço profundo para alocação dinâmica de recursos em redes virtualizadas. O método aprende políticas de mapeamento através de interação com o ambiente, adaptando-se a mudanças no estado da rede. Contudo, a abordagem requer extenso período de treino e pode sofrer com instabilidade de convergência em ambientes complexos.

Wang et al.~\cite{tsc-2023-hrl-acra} propõem \textit{HRL-ACRA}, um método de aprendizado por reforço hierárquico que realiza controle de admissão e alocação de recursos conjuntamente. A abordagem utiliza decomposição hierárquica para reduzir a complexidade do espaço de estados. Apesar de resultados promissores, o método é treinado de forma fim-a-fim para um único algoritmo, não explorando a complementaridade de múltiplos métodos existentes.

Mais recentemente, Wang et al.~\cite{ijcai-2024-flagvne} apresentam \textit{FlagVNE}, um \textit{framework} flexível de aprendizado por reforço para alocação de recursos em redes. O método generaliza bem para diferentes topologias e padrões de carga, mas requer retreino significativo ao incorporar novos objetivos ou restrições. Além disso, abordagens de aprendizado por reforço tipicamente demandam milhões de episódios de treino, tornando-as computacionalmente custosas.

\subsection{Seleção de Algoritmos}

Rice~\cite{rice1976algorithm} formalizou o problema de seleção de algoritmos, estabelecendo o \textit{framework} composto por espaços de problemas, algoritmos, \textit{features} e desempenho. O trabalho demonstra que características da instância determinam qual método será mais eficaz, motivando abordagens adaptativas ao invés de soluções únicas.

Xu et al.~\cite{xu2012satzilla} desenvolvem \textit{SATzilla}, um seletor de algoritmos baseado em portfólio para problemas de satisfabilidade booleana (SAT). O sistema extrai \textit{features} de instâncias SAT e utiliza modelos de aprendizado de máquina para predizer o melhor algoritmo dentre um portfólio. \textit{SATzilla} demonstra ganhos significativos de desempenho em relação a algoritmos individuais, validando a eficácia de seleção adaptativa. O sucesso em SAT motiva aplicações similares em outros domínios como VNE.

\subsection{Análise Comparativa}

A Tabela~\ref{tab:related-work} apresenta uma comparação sistemática entre os trabalhos relacionados e a abordagem proposta, destacando as principais características e diferenças.

\begin{table*}[t]
\centering
\caption{Comparação entre Trabalhos Relacionados e Abordagem Proposta}
\label{tab:related-work}
\begin{tabular}{lcccccc}
\toprule
\textbf{Trabalho} & \textbf{Tipo} & \textbf{Adaptativo} & \textbf{Múltiplos} & \textbf{ML} & \textbf{Tempo} & \textbf{Interpretável} \\
                  & \textbf{Algoritmo} & \textbf{por VNR} & \textbf{Algoritmos} & \textbf{Supervisionado} & \textbf{Treino} & \\
\midrule
Cao et al.~\cite{cao2018nfv-deeprl} & DRL & Sim & Não & Não & Alto & Não \\
HRL-ACRA~\cite{tsc-2023-hrl-acra} & DRL Hierárquico & Sim & Não & Não & Alto & Não \\
FlagVNE~\cite{ijcai-2024-flagvne} & DRL & Sim & Não & Não & Muito Alto & Não \\
\midrule
\textbf{Este Trabalho} & \textbf{Seletor XGBoost} & \textbf{Sim} & \textbf{Sim} & \textbf{Sim} & \textbf{Baixo} & \textbf{Sim} \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{Diferencial da Abordagem Proposta}: Este trabalho distingue-se por combinar múltiplos algoritmos existentes através de seleção inteligente baseada em \textit{XGBoost}, ao invés de desenvolver um único algoritmo novo. A abordagem oferece vantagens significativas:

\begin{itemize}
    \item \textbf{Adaptatividade}: Seleciona o algoritmo mais apropriado para cada VNR baseado em suas características específicas, ao contrário de algoritmos tradicionais que mantêm estratégias fixas independentemente do contexto~\cite{fischer2013vne-survey}.

    \item \textbf{Eficiência de Treino}: Utiliza aprendizado supervisionado com dados pré-computados, requerendo apenas trinta segundos de treino versus milhões de episódios em métodos de aprendizado por reforço~\cite{cao2018nfv-deeprl,tsc-2023-hrl-acra,ijcai-2024-flagvne}.

    \item \textbf{Interpretabilidade}: Fornece análise de importância de \textit{features}, permitindo compreender quais características (topologia, tamanho) direcionam decisões de seleção, diferentemente de redes neurais profundas que funcionam como "caixa preta".

    \item \textbf{Extensibilidade}: Facilmente incorpora novos algoritmos ao portfólio sem necessidade de retreino completo do modelo, apenas atualização incremental com novos dados.

\end{itemize}

A principal contribuição metodológica é a aplicação bem-sucedida do paradigma de seleção de algoritmos, comprovado em SAT~\cite{xu2012satzilla}, ao domínio de VNE, demonstrando que a heterogeneidade de características de VNRs e estados de rede justifica estratégias adaptativas.

\section{Metodologia}
\label{sec:Proposal}

Esta seção descreve a metodologia proposta para seleção dinâmica de algoritmos VNE usando \textit{XGBoost}, incluindo o processo de geração de simulações, engenharia de \textit{features}, treino do modelo e estratégia de avaliação.
...
\begin{enumerate}
    ...
    \item \textbf{Treino do Modelo}: Otimizar hiperparâmetros do \textit{XGBoost} e treinar classificador multiclasse
...
A metodologia proposta utiliza um \textit{pipeline} de aprendizado de máquina composto por cinco etapas sequenciais (Figura~\ref{fig:pipeline}):

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=2.5cm]
        \node (step1) [draw, rectangle, minimum width=2cm, minimum height=0.8cm] {\textbf{1. Extrair}};
        \node (step2) [draw, rectangle, right of=step1, minimum width=2cm, minimum height=0.8cm] {\textbf{2. Preparar}};
        \node (step3) [draw, rectangle, right of=step2, minimum width=2cm, minimum height=0.8cm] {\textbf{3. Treinar}};
        \node (step4) [draw, rectangle, right of=step3, minimum width=2cm, minimum height=0.8cm] {\textbf{4. Analisar}};
        \node (step5) [draw, rectangle, right of=step4, minimum width=2cm, minimum height=0.8cm] {\textbf{5. Avaliar}};
    ...
    \end{tikzpicture}
    \caption{\textit{Pipeline} de Aprendizado de Máquina para Seleção de Algoritmos VNE}
    \label{fig:pipeline}
\end{figure}

\textbf{Etapa 1 - Extração de Dados de Simulações}:
...
Para cada combinação de algoritmo e topologia, foram executadas dez repetições com diferentes sementes aleatórias para garantir significância estatística. Cada simulação gerou aproximadamente cem VNRs, totalizando 11.672 execuções de algoritmos. Após deduplicação (removendo múltiplas execuções da mesma VNR), foram obtidas oito mil VNRs únicas para treino. Os registros de cada simulação foram extraídos em formato CSV, contendo:

\begin{itemize}
    \item Identificadores de VNR: ID único, topologia, semente aleatória
    \item Características da VNR: número de nós, número de enlaces, demandas de recursos
    \item Estado da rede: utilização de nós, utilização de enlaces, recursos disponíveis
    \item Resultado da execução: aceita/rejeitada, tempo de processamento, receita gerada
\end{itemize}
...
\textbf{Saída da Etapa 1}: arquivo CSV com aproximadamente 14.000 registros brutos (1.000 VNRs × 8 algoritmos × 1-2 sementes).
...
\subsection{Treino do Modelo \textit{XGBoost}}

O \textit{dataset} de oito mil VNRs foi dividido em três conjuntos:
\begin{itemize}
    \item \textbf{Treino}: 5600 VNRs (70\%)
...
O modelo foi treinado para classificação multiclasse com objetivo de maximizar o \textit{F1-score} ponderado. O tempo total de treino foi de aproximadamente trinta segundos em um \textit{MacBook Air} com processador \textit{Apple} M1 (2024) e 16GB de RAM.
...
Todo o código fonte, \textit{datasets}, modelos treinados e \textit{scripts} de análise desenvolvidos neste trabalho estão disponíveis publicamente no repositório GitHub:

\begin{center}
\url{https://github.com/luismomm2110/virne/tree/setup-topologies}
\end{center}

O repositório contém:
\begin{itemize}
    \item Implementação completa da plataforma \textit{ViRNE} com os sete algoritmos avaliados
    \item \textit{Scripts} para geração de topologias (\textit{Tree} e \textit{Fat-Tree})
    \item Código de extração de \textit{features} e engenharia de dados
    \item Modelo \textit{XGBoost} treinado e otimizado
    \item \textit{Notebooks} de análise e geração de gráficos
    \item Dados de simulação e resultados experimentais
    \item Documentação completa de reprodução dos experimentos
\end{itemize}
