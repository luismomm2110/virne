...
Figure~\ref{fig:tree-structure} illustrates the general structure of a decision tree: the tree starts at a root node containing all training data.
...
\begin{figure}[H]
    ...
    \caption{Structure of a decision tree: recursive partitioning of the feature space.}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{tree_example_path}
    \caption{Example of decision path (root to leaf) in tree for VNE algorithm selection.}
    \label{fig:tree-example-path}
\end{figure*}

Unlike deep neural networks or reinforcement learning models, each step is comprehensible and auditable by human operators.
Decision trees are particularly suitable for the VNE algorithm selection problem due to the following characteristics:

\begin{itemize}
    \item Interpretability: Each decision can be traced through an explicit logical path in the tree.
    \item Scale Invariance: Work adequately with features at different scales (CPU in units, utilization in percentage).
    \item Low Latency: Inference has $O(\log n)$ complexity in depth, typically below 1\,ms.
    \item Explainability: Operators understand exactly which features determined the decision.
\end{itemize}
...
VNE algorithm selection cannot be based on a single performance metric. Different operational contexts (high load, limited resources, latency requirements) require different trade-offs. Figure~\ref{fig:algorithm-comparison} presents a comprehensive analysis of four complementary metrics for the eight evaluated algorithms:
...
Figure~\ref{fig:real-performance-improvement} presents the results of real operational performance when decision trees are used for algorithm selection. Results show that contextual selection via decision trees produces substantial improvements in real operational performance in all objectives:
...
\textit{No Algorithm is Universally Optimal}: MIP maximizes acceptance rate (36.8\%), GA\_META maximizes revenue (400.628), D\_ROUND offers best cost-benefit ratio (246.0), and PSO\_META is fastest (0.31s). Each algorithm excels in a different dimension.

\textit{Explicit Trade-offs}: MIP offers high acceptance but with elevated processing time (1.03s). D\_ROUND is very fast but with reduced acceptance (20.4\%). GA\_META offers adequate balance between revenue and processing time.

\textit{Justification for Multi-Objective Approach}: This performance variation empirically evidences that:
\begin{itemize}
...
This work proposed an approach for zero-touch selection of Virtual Network Embedding algorithms using multi-objective decision trees, considering virtual network request characteristics and physical infrastructure state.
The approach differentiates itself from the alternatives based on deep learning by offering complete interpretability through explicit decision paths and inference latency below 1 \,ms, allowing real-time online operation.
...
\begin{itemize}
    \item Acceptance Rate Improvement: Increase of +89.83\% compared to fixed algorithm baseline (from 43.17\% to 81.95\%)
    \item Top-3 Accuracy: Between 79.1\% and 85.9\%, showing robustness in predicting the best algorithm
    \item Revenue-Cost Ratio Improvement: Increase of +58.76\% (from 0.4574 to 0.7262)
    \item Average Revenue Improvement: Increase of +61.40\% (from 70.29 to 113.45 per VNR)
\end{itemize}
