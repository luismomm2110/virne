"""
PSO-Meta experiments on Tree 2x topology (64 nodes).

This script runs the PSO-Meta solver on the larger Tree 2x topology
for comparative analysis with PL-Rank.

Usage:
    python main_tree_2x_pso_meta.py p_net_setting=tree_2x_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_tree_2x_pso_meta")
def run(config: DictConfig):
    """
    Run the VNE simulation with Tree 2x topology.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running PSO-Meta solver on Tree 2x topology (64 nodes)")
